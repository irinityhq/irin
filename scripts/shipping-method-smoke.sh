...
# Static guards: fail closed on real live invocations in the driver + W5 children.
# Comments, prose (fail/pass/printf/...), meta-greps, and re.compile pattern
# lines are ignored — only command-shaped uses trip the guard.
section "W5 smoke static guards"
...
# Leading boundary: line start, operator, or whitespace before an invocation.
_b = r"(?:^|[;&|`(\n]|\s)"
forbidden = [
    ...
    # upload PATH then --clobber (not "grep ... --clobber" alone)
    re.compile(_b + r"gh\s+release\s+upload\s+\S+[^\n]*--clobber\b"),
]
# Driver must not call release-transaction at all (fake-gh child owns hermetic publish).
...
PY
pass "static guards (no Apple/prepare/clobber in driver+children; hermetic children wired)"
