...
#
# GitHub Actions supports concurrency.queue:max (2026-05). Pinned actionlint
# 1.7.12 predates that key. The installed `actionlint` entrypoint is a thin
# wrapper that ignores only that one schema gap so ordinary PR CI (which runs
# ci.yml@main steps against the PR checkout) and local lint stay green until
# upstream actionlint learns the field. Every other finding still fails closed.
set -euo pipefail
...
real_destination="$bin_dir/actionlint-${version}-real"
wrapper_destination="$bin_dir/actionlint"
# Match actionlint's "unexpected key \"queue\" for \"concurrency\" section" only.
queue_ignore='unexpected key "queue" for "concurrency" section'

wrapper_body="$(cat <<EOF
#!/usr/bin/env bash
# Generated by scripts/bootstrap-actionlint.sh — do not edit.
set -euo pipefail
exec "$real_destination" -ignore '$queue_ignore' "\$@"
EOF
)"
