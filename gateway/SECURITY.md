...
re-exposes a fixed allow-list of admin routes on the loopback HTTP port, and the
...
and are admin-authorized in the sidecar. The producer arm ceremony is proxied only when the installed
desktop pack mounts its non-secret feature marker. Externally reachable Gateway
