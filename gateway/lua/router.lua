...
--
-- Route families (see _M.route):
--   /v1/batches*   — provider batch passthrough        (route_batch)
--   /v1/ otherwise — governed chat + Responses traffic (route_chat)
--   /v1/models     — authenticated catalog             (_M.models)
-- ==========================================================================
...
-- Host-side CLI proxies authenticate with a shared-secret bearer (constant-
-- time compare in the proxy). The token env var is a per-provider lookup
-- fact; when unset the loopback-only proxy disables auth and no header
-- is injected.
local CLI_PROXY_TOKEN_ENV = {
    ["claude-cli"] = "CLAUDE_PROXY_TOKEN",
    ["gpt-cli"]    = "CODEX_PROXY_TOKEN",
    ["gemini-cli"] = "GEMINI_PROXY_TOKEN",
}
...
--- Read the request body, falling back to the body file when the request
--- exceeded client_body_buffer_size. Returns "" when nothing could be
--- read; callers own empty-body policy (the main path 400s, the batch
--- passthrough forwards empty bodies for GET/DELETE-shaped operations).
local function read_body_with_file_fallback()
    ngx.req.read_body()
...
--- Terminate the audit chain and emit the paired JSON error. Every
--- request_received row pairs with exactly one terminating row; the
--- schedule marks record.chain_terminated so later phases (and cost.lua's
--- log handler) cannot double-terminate. payload_extra / meta_extra must
--- contain only frozen scalars captured at the call site — the closure
--- runs in a timer after the access phase has exited. Returns true so the
--- caller can stop its own phase sequence.
local function reject(record, action, decision, payload_extra, meta_extra,
                      status, message, err_type, err_code)
    local request_id = record.request_id
    local alias      = record.alias
    local caller_key = record.caller_key
    local payload = { request_id = request_id }
    if payload_extra then
        for k, v in pairs(payload_extra) do payload[k] = v end
    end
    local meta = { action = action, decision = decision, request_id = request_id }
    if meta_extra then
        for k, v in pairs(meta_extra) do meta[k] = v end
    end
    schedule_terminator(record, action, request_id, function(premature)
        if premature then return end
        ledger_record("gateway", alias, payload, meta, caller_key)
    end)
    json_error(status, message, err_type, err_code)
    return true
end

--- Point the upstream request at the provider's credential. Covers the
--- three auth-header styles: Authorization bearer (default), x-api-key
--- (Anthropic), and a custom header such as council's X-Gateway-Auth,
--- which takes the provider's auth_prefix verbatim ("" = raw token).
--- Callers own stripping client-supplied credentials beforehand.
local function apply_provider_auth(provider, api_key)
...
local function route_batch(headers)
    ...
    -- For methods with a body (POST/PATCH/PUT) read it; GET status/output
    ...
    if request_method == "POST" or request_method == "PUT" or request_method == "PATCH" then
        raw_body = read_body_with_file_fallback()
    end
    ...
    apply_provider_auth(provider, api_key)
...
--- over-depth multimodal nesting (spec §12.6 / P1 #18); route_chat
--- surfaces that as a 400 instead of silently treating the request as
--- text-less.
local function extract_last_user_content(req)
    if req.messages then
        for i = #req.messages, 1, -1 do
            if req.messages[i].role == "user" then
                return extract_text_content(req.messages[i].content)
            end
        end
    elseif type(req.input) == "string" then
        return req.input
...
--- are NEVER trusted on entry. Stash their values, clear them from the
--- request so no downstream phase (or upstream provider call) can observe
--- them, and only restore them (restore_council_headers) if the
--- auth-resolved key has service_role == "council" AND
--- key_id == COUNCIL_GATEWAY_KEY_ID. Two factors required: an admin can't
--- elevate a key just by setting service_role; the key_id must ALSO match
--- the env var. See §5.6 for the rationale.
local function strip_council_depth_headers()
    local pre_strip_headers = ngx.req.get_headers()
    local stashed = {
        depth         = pre_strip_headers["x-council-depth"],
        session_id    = pre_strip_headers["x-council-session-id"],
        request_id    = pre_strip_headers["x-council-request-id"],
        parent_req_id = pre_strip_headers["x-parent-request-id"],
        transport_id  = pre_strip_headers["x-council-transport-id"]
                        or pre_strip_headers["x-council-original-provider"],
    }
...
---
--- Returns whether the caller is the trusted council service, and whether
--- a client-supplied exact-transport identity was refused: such a request
--- is never reinterpreted as an ordinary smart route, so setup/key drift
--- becomes a loud failure instead of silently changing the provider behind
--- a governed Council proceeding.
local function restore_council_headers(record, stashed)
    ...
    return is_council_service, transport_restore_denied
end

--- Input guard (decon). MCP passthrough (loopback + X-Tool-Format: mcp) and
--- council-internal traffic (service-role council key with a restored
--- parent id — an internal LLM→LLM scan whose synthesized transcript would
--- false-positive the external-input tuner) bypass the guard, with a metric
--- each. Returns true only when the request was blocked and terminated.
local function enforce_input_guard(record, last_user_content, council_internal)
    ...
    if not last_user_content or record.mcp_passthrough or council_internal then
        return false
    end
    ...
    if guard_result and guard_result.blocked then
        ...
        return reject(record, "guard_input", "blocked", nil,
                      { reason = guard_result.blocked_reason },
                      403, "request blocked by security guard: " ..
                          (guard_result.blocked_reason or "threat detected"),
...
--- STEP 2: Cache check — short-circuit on hit. Returns true after serving
--- the cached response (request finalized); false on miss (or skipped
--- cache), letting the caller proceed to routing.
---
--- Streaming requests skip cache entirely — a cached JSON response served
--- to an SSE-expecting client would break the wire format. Cache key is
--- hashed from (alias, sensitivity, raw_body) on the Rust side. Hashing
...
---
--- Council models (council-triage, council-warroom) own idempotency via
--- the peek/lock/claim path in council_endpoint_gate; L1 cache would
--- absorb identical-body replays without writing the council_replay
--- ledger row or surfacing X-Idempotency-Replay, so it is skipped for
--- them.
local function try_serve_cached(record, is_responses_api)
    local is_council_model = record.alias and string.sub(record.alias, 1, 8) == "council-"

    local cache_result
    if record.is_streaming then
    ...
    if not record.is_streaming then
        local m = ngx.shared.gw_metrics
...
--- An authenticated Council transport request is an exact dispatch, not a
--- routing hint: the requested alias resolves directly so smart routing
--- and model fallback cannot silently change the pair.
---
...
---
--- Returns model_cfg, resolved_name, routing. A nil model_cfg means the
--- request was terminated (sidecar unreachable, or unknown model).
local function resolve_route(record, req, sovereign_mode)
    ...
    elseif routing then
        ...
        reject(record, "route_decide", "rejected", nil,
               { reason = "sidecar_unreachable", sensitivity = record.sensitivity },
               503, "sovereign routing unavailable",
    ...
    if not model_cfg then
        ...
        reject(record, "route_decide", "rejected", nil,
               { reason = "unknown_model", sensitivity = record.sensitivity },
               400, "unknown model: " .. record.alias, nil, "ERR_MODEL_UNKNOWN")
        return nil
    end

    return model_cfg, resolved_name, routing
end

--- Exact-transport enforcement for authenticated Council transports.
--- Sovereign mode must stay on local providers, and the requested
--- transport must be able to serve the resolved model. Returns true when
--- the request was terminated.
local function enforce_transport_policy(record, model_cfg, sovereign_mode)
    if record.requested_transport == "" then return false end

    if sovereign_mode and not council_transport.is_local_provider(model_cfg.provider) then
        return reject(record, "route_decide", "rejected", nil,
            { reason              = "sovereign_transport_external",
              requested_transport = record.requested_transport,
              resolved_provider   = model_cfg.provider },
            403, "requested Council transport is not local in Sovereign mode",
    ...
    if not transport_ok then
        return reject(record, "route_decide", "rejected", nil,
            { reason              = transport_reason,
              requested_transport = record.requested_transport,
              resolved_provider   = model_cfg.provider },
            400, "requested Council transport '" .. record.requested_transport
...
---   vertex:       swap path to streamGenerateContent?alt=sse (dispatch_upstream)
...
--- Returns true when the request was terminated.
local function apply_streaming_provider_rules(req, model_cfg, is_streaming)
    if not is_streaming then return false end
...
--- Estimates 1K output tokens at the resolved model's output pricing.
--- Returns true when the request was blocked and terminated.
local function check_budget(record, model_cfg)
    if not (record.budget_key and record.budget_key ~= "") then
        return false
    end
    ...
    if budget_result and not budget_result.allowed then
        ...
        return reject(record, "budget_check", "blocked",
                      { budget_key = record.budget_key },
                      { budget_key = record.budget_key,
                        reason     = budget_result.reason },
                      429, "budget exceeded: " .. (budget_result.reason or "limit reached"),
                      "budget_exceeded", "ERR_BUDGET_EXCEEDED")
    end
    return false
end
...
local function check_policy(record, model_cfg)
    ...
    if policy_result and not policy_result.allowed and not policy_result.dry_run then
        ...
        return reject(record, "policy_evaluate", "blocked",
                      { provider = model_cfg.provider,
                        level    = policy_result.level },
                      { provider    = model_cfg.provider,
                        sensitivity = record.sensitivity,
                        reason      = policy_result.reason },
                      403, "sensitivity policy violation: " ..
...
---   2. Streaming rejected BEFORE depth/idem/lock/claim — otherwise a
---      streaming request would acquire a concurrency slot and leak a
---      Pending entry before bouncing at translate time (spec §7 / §9
---      test #4; the council translator's streaming_unsupported only
---      fires post-mutation).
---   3. Re-entry rejection: any X-Council-Depth >= 1 that survived the
...
---   4. Idempotency-Key required (Stripe semantics), body-hash
---      peek/lock/claim. The replay branch ngx.exit()s from the access
---      phase via cost.account_replay, mirroring the cache-hit pattern.
---
--- Returns true when the request was terminated here; false lets the
--- caller continue to dispatch with the concurrency slot + Pending entry
--- held.
local function council_endpoint_gate(record, model_cfg, resolved_name, is_streaming)
    if model_cfg.provider ~= "council" then return false end
    ...
    if depth >= 3 then
        json_error(409, "Council depth hard cap exceeded.",
            "invalid_request_error", "ERR_COUNCIL_DEPTH_CAP")
        return true
    end
    if depth >= 1 then
        json_error(409, "Recursive council invocation blocked.",
    ...
    if peek_err then
        ...
        json_error(503, "council idempotency check unavailable",
...
--- STEP 6: Resolve provider, translate the body to provider-native format
--- (Rosetta layer), and wire the upstream proxy_pass target. The final
--- phase of route_chat — after it returns, nginx proxies to
--- ngx.var.target_url with the rewritten request.
local function dispatch_upstream(record, req, model_cfg, resolved_name,
                                 routing, is_streaming, sovereign_mode)
    ...
    --
    -- Spec §5.2a (P0 #2 regression guard): the generalized branching in
    -- apply_provider_auth covers providers that use neither
    -- `Authorization` nor `x-api-key` (e.g. council's X-Gateway-Auth).
    -- Anthropic + xAI smoke regression is covered by spec §9 test #18.
    apply_provider_auth(provider, api_key)

    -- Set provider-specific extra headers (e.g. anthropic-version). Applied
    -- AFTER translator extras so provider config wins on any collision.
    ...
    -- present. The token is set in the proxy process env on the host; the
    -- proxy enforces with constant-time compare. When unset (loopback-only
    -- proxy), the proxy disables auth and we skip the header.
    local token_env = CLI_PROXY_TOKEN_ENV[model_cfg.provider]
    if token_env then
        local tok = os.getenv(token_env)
        if tok and tok ~= "" then
...
--- Main /v1/ path: chat-completions + Responses API traffic. Runs the
--- access phases in wire order; each phase names its helper.
local function route_chat()
    -- =====================================================================
    ...
    -- =====================================================================
    local raw_body = read_body_with_file_fallback()
    if raw_body == "" then
    ...
    -- Extract the last user message's text content (see
    -- extract_last_user_content) and surface over-depth nesting as a 400.
    ngx.ctx.gw_content_depth_exceeded = nil
    local last_user_content = extract_last_user_content(req)
    if ngx.ctx.gw_content_depth_exceeded then
        return json_error(400, "Multimodal content nesting exceeded depth cap.",
                          "invalid_request_error", "ERR_CONTENT_DEPTH_EXCEEDED")
    end

    -- §5.6 capture-strip of client-supplied council identity headers.
    local stashed = strip_council_depth_headers()
    ...
    -- =====================================================================
    -- STEP 1: AUTHENTICATION & RATE LIMITING — shared helper, the same
    -- gating as the batch passthrough and the /v1/models catalog.
    -- =====================================================================
    local auth_ok, _, auth_result = authenticate_request(headers)
    ...
    -- §5.6 restore for the trusted council service identity.
    local _, transport_restore_denied = restore_council_headers(record, stashed)
    ...
    if transport_restore_denied then
        return reject(record, "route_decide", "rejected", nil,
            ...
            403, "Council transport identity was not authorized for this key",
    ...
    -- service-role callers), decon is bypassed — see enforce_input_guard
    -- for the rationale.
    local council_internal = record.parent_council_request_id ~= nil
                             and record.parent_council_request_id ~= ""
    if enforce_input_guard(record, last_user_content, council_internal) then return end

    -- STEP 2: Cache (short-circuit on hit).
    if try_serve_cached(record, is_responses_api) then return end

    -- STEP 3: Smart routing / exact-transport dispatch.
    local model_cfg, resolved_name, routing = resolve_route(record, req, sovereign_mode)
    if not model_cfg then return end

    -- Exact-transport enforcement, then per-provider streaming rules.
    if enforce_transport_policy(record, model_cfg, sovereign_mode) then return end
    if apply_streaming_provider_rules(req, model_cfg, is_streaming) then return end

    -- STEP 4 + STEP 5: budget pre-flight, sensitivity firewall.
    if check_budget(record, model_cfg) then return end
    if check_policy(record, model_cfg) then return end

    -- STEP 5b: council endpoint gate (idempotency + concurrency).
    if council_endpoint_gate(record, model_cfg, resolved_name, is_streaming) then return end

    -- STEP 6: translate + wire the upstream proxy target.
    dispatch_upstream(record, req, model_cfg, resolved_name, routing,
                      is_streaming, sovereign_mode)
end

--- Access-phase entry point. Explicit route-family dispatch: the batch
--- passthrough and the governed main path intentionally apply different
--- phase subsets (see route_batch / route_chat); /v1/models is served by
--- _M.models on its own location block.
function _M.route()
    local uri = ngx.var.uri or ""
    if uri == "/v1/batches" or uri:sub(1, #"/v1/batches/") == "/v1/batches/" then
        return route_batch(ngx.req.get_headers())
    end
    return route_chat()
end
