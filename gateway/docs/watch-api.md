...
- **Canary:** when `WATCH_CANARY_TENANT` is set (the base compose defaults it to `sovereign`; the canary overlay, desktop pack, and verify/demo stack set `canary`), tenant-scoped admin paths reject other tenants with `403 single_tenant_violation`. Unset → default tenant name `sovereign` for the tripwire config.
