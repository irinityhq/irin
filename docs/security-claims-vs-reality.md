...
| Private remote Web access | Optional | Installed IRIN.app Settings configures Tailscale Serve when the local client is installed and connected; it publishes to the operator's tailnet only, never to the public internet (Funnel is never configured by IRIN). Source `make setup` / runtime never applies or disables Serve. |
