...
| Stale claim under a confirmed ceiling | Not enforced in v1 (known limitation) | Watch `reserve` does not assert that the caller holds the live writer claim (`instance_id == writer_claim.holder`) before spending under a confirmed ceiling. A second Gateway process with a stale claim can reserve against the same cap; the ledger day-cap still bounds total spend, but per-holder attribution is not proven. Adding an ownership gate to `reserve` in v1 would break the multiprocess ledger-cap proof; the fix needs a shared writer-identity seam or a per-tenant ledger partition. No regression test exists for this gap yet; it was previously a placeholder `#[ignore]` test, now removed. |
...
| Autonomous Worker execution | Not an operator feature (default off) | This row records the shipped default, not a design ceiling. The current default operator path ends at a signed Outbox directive. Authenticated claim, heartbeat, ack, worker-ack, and nack management routes are mounted. The built-in worker loop that uses them is disabled by default and is not an operator-ready autonomous executor. |
