...
The IRIN CI workflow has no branch `push` trigger. Merging a pull request does
not repeat the same full proof after merge. A manual full proof is available
when a fresh integrated result is needed.
...
- War Room web source and npm lockfiles select Linux web lint, typecheck,
  hosted and embedded builds, unit tests, and real browser tests. They do not
...
- Tauri source selects the native-shell smoke on `macos-15`. Its manifest or
  ...
  deny policy also selects that supply-chain lane. When both Web and Tauri are
  selected, only the macOS lane produces the embedded Web export so identical
  Next builds do not run concurrently.
